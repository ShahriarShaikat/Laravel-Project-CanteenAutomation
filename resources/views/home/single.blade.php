		...
		<div class="menu">
			<ul>
			  <li><a href="{{route('profile.page')}}">Profile</a></li>
				<li class="current"><a href="{{route('home.userhomepage')}}">Blog</a></li>
	...
	<div class="wrap-content zerogrid">
		<div class="row block03">
            ...
            @elseif($com->totime >= 60 && $com->totime < 3600)
					...
					<div class="comment_{{$com->cid}}">
						  <a style="cursor:pointer;" class="edit">Edit</a>
						  <a style="margin-left:9px;"  href="{{route('delete.comment',['id'=>$com->cid,'post_id'=>$com->pid])}}">Delete</a><br>
						  	<script>
		                      jQuery(document).ready(function()
		                      {

			                     jQuery(".comment_{{$com->cid}} .edit").click(function()
			                     {
			                     	
				                    jQuery(".comment_{{$com->cid}} form").toggle();
			                     });
					  ...
					  <form id="com-form" action="{{route('comment.edit',['id'=>$com->cid,'post_id'=>$com->pid])}}" method="post">
					      {{@csrf_field()}}
					      
						  <textarea id="text" placeholder="Write something...." rows="5" cols="60" type="text"  name="edit_comment">{{$com->comment}}</textarea><br>
						  <button type="submit" name="submit" id="buttonn">Update Comment</button>
					  </form>
					</div>
					  @endif
					  </div>
